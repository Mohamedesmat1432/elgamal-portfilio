<div>
    @can('branch-restore')
        <livewire:trash.branches.branch-restore />
    @endcan

    @can('branch-bulk-restore')
        <livewire:trash.branches.branch-bulk-restore />
    @endcan

    @can('branch-force-delete')
        <livewire:trash.branches.branch-force-delete />
    @endcan

    @can('branch-force-bulk-delete')
        <livewire:trash.branches.branch-force-bulk-delete />
    @endcan
    ...
    <x-table>
        ...
        <x-slot name="thead">
            <tr>
                ...
                <th class="px-6 py-4">
                    <button wire:click="sortBy('descreption')" class="flex justify-center w-full">
                        {{ __('trans.descreption') }}
                        <x-sort-icon field="descreption" :sort_by="$sort_by" :sort_dir="$sort_dir" />
                    </button>
                </th>
                <th class="px-6 py-4">
        ...
        <x-slot name="tbody">
            @forelse ($this->branches() as $branch)
                <tr wire:key="trash-branch-{{ $branch->id }}" class="border-b border-neutral-200">
                    ...
                    <td class="px-6 py-4">
                        @can('branch-restore')
                            <x-icon name="arrow-uturn-left" class="w-10 h-10 inline-block cursor-pointer"
                                x-on:click.prevent="$dispatch('restore-modal', {id: '{{ $branch->id }}', name: '{{ $branch->name }}'})" />
                        @else
                        ...
                        @can('branch-force-delete')
                            <x-icon name="trash" class="w-10 h-10 text-red-600 inline-block cursor-pointer"
                                x-on:click.prevent="$dispatch('force-delete-modal', {id: '{{ $branch->id }}', name: '{{ $branch->name }}'})" />
                        @else
